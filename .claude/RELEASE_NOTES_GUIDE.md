...
**使うとき**: 新機能・バグ修正・改善が複数あり、網羅的に伝えたい場合

```
Claude Code {version}

【新機能】
・/loop コマンドを追加。プロンプトまたはスラッシュコマンドを定期的な間隔で繰り返し実行できる（例：/loop 5m check the deploy）
・セッション内で定期的なプロンプトを実行するためのcronスケジューリングツールを追加
・voice:pushToTalk キーバインドを追加。音声起動キーを keybindings.json で変更可能に

【バグ修正】
・長時間セッションのstdinフリーズを修正
・Voice有効時の起動5〜8秒フリーズを解消
・OAuth トークン更新時の再接続失敗を修正

【改善】
・起動時間の改善（ネイティブ画像処理の遅延読み込み）
・ブリッジセッション再接続の高速化
```

**ポイント**:
- CHANGELOG の Added/Fixed/Improved/Removed をそのまま【新機能】【バグ修正】【改善】【削除】に対応させる
- 各項目は日本語で、自分の言葉で書く（英語をそのまま翻訳しない）
- 重要度順に並べる
